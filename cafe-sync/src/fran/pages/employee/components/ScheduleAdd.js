    ...
    const hasDuplicate = workers.some(
      (worker, index, self) =>
        ...
        self.some(
          (w, i) =>
            i !== index &&
            w.empCode === worker.empCode &&
            w.division === worker.division
        )
    );

    const hasDBDuplicate = workers.some((worker) =>
      existingSchedules.some((schedule) => {
        const scheduleDateFormatted = new Date(schedule.date)
          .toISOString()
          .split("T")[0];
        const workerDateFormatted = new Date(worker.scheduleDate)
          .toISOString()
          .split("T")[0];

        return (
          String(schedule.emp) === String(worker.empCode) &&
          Number(schedule.extendedProps.scheduleDivision) ===
            Number(worker.division) &&
