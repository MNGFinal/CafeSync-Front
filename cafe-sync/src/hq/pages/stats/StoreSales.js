...
const StoreSales = ({ startDate, endDate, searchTrigger }) => {
  // ✅ 조회 버튼이 눌릴 때만 실행
  ...
  useEffect(() => {
    ...
    console.log(
      `📌 [DEBUG] Fetching store sales for ${startDate} ~ ${endDate}`
    );
        ...
        {stores.length > 0 ? (
          stores.slice(0, 6).map((store, index) => (
            <div key={store.franCode} className={styles.storeBox}>
              <img
                src={store.franImage || "/default-image.jpg"}
                alt={`${store.franName} 이미지`}
                className={styles.image}
              />
              <h3>
                {index + 1}등 {store.franName}
              </h3>
              <p className={styles.saless}>
                매출액: {store.totalSales?.toLocaleString()}원
              </p>
            </div>
          ))
        ) : (
